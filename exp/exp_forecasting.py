...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        if configs.llm_model == "LLAMA2":
            ...
            self.config = LlamaConfig.from_pretrained(
                llama3_path, token=self.hug_token, cache_dir=cache_path
            )
            ...
            self.llm_model = LlamaModel.from_pretrained(
                llama3_path,
                config=self.config,
                token=self.hug_token,
                cache_dir=cache_path,
            )
            self.tokenizer = AutoTokenizer.from_pretrained(
                llama3_path, use_auth_token=self.hug_token, cache_dir=cache_path
            )
        ...
        print(
            self.llm_model
        )  # Assuming the LLM model is stored as 'llm_model' attribute
        ...
        self.text_to_output_proj = nn.Linear(
            self.text_embedding_dim, self.args.c_out
        ).to(self.device)
    ...
    def _select_optimizer_proj(self):
        model_optim = optim.Adam(
            self.mlp_proj.parameters(), lr=self.args.learning_rate3
        )
        return model_optim
    ...
    def _select_optimizer_weight(self):
        model_optim = optim.Adam(
            [
                {"params": self.weight1.parameters()},
                {"params": self.weight2.parameters()},
            ],
    ...
    def _select_optimizer_text_proj(self):
        model_optim = optim.Adam(
            self.text_to_output_proj.parameters(), lr=self.args.learning_rate3
        )
        return model_optim
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(
                ...
                prior_y = (
                    torch.from_numpy(vali_data.get_prior_y(index))
                    .float()
                    .to(self.device)
                )

                prompt_emb = (
                    vali_data.get_text_embeddings(index).float().to(self.device)
                )
                # decoder input
                ...
                outputs, _ = self.model(
                    batch_x, batch_x_mark, prompt_emb, dec_inp, batch_y_mark, prior_y
                )
    ...
    def train(self):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(
                ...
                # 0523
                prior_y = (
                    torch.from_numpy(train_data.get_prior_y(index))
                    .float()
                    .to(self.device)
                )
                ...
                prompt_emb = (
                    train_data.get_text_embeddings(index).float().to(self.device)
                )
    ...
    def analyze_train_attention(self, batch_index=0):
        ...
        """
        print("🔍 Starting Getting Train Attention Analysis...")
        print(
            f"📊 Analyzing batch {batch_index} from train dataset with batch-size is {self.args.batch_size}"
        )
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(
                ...
                _, attention_weights = self.model(
                    batch_x, batch_x_mark, prompt_emb, dec_inp, batch_y_mark
                )
    ...
    def test(self, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, index) in enumerate(
                ...
                prior_y = (
                    torch.from_numpy(test_data.get_prior_y(index))
                    .float()
                    .to(self.device)
                )
                ...
                prompt_emb = (
                    test_data.get_text_embeddings(index).float().to(self.device)
                )
        ...
        self.metrics_tracker.log_test_metrics(
