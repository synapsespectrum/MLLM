...
for data_path in $datasets; do
    ...
    target="OT"
    log_file="$log_dir/${data_path}_${seq_len}.log"
    echo ""
    echo "=================================================="
    echo "Processing dataset: $data_path (freq: $freq, seq_len: seq_len)"
    echo "Target: $target"
    echo "=================================================="
    python generate_embedding.py \
        ...
        > "$log_file" 2>&1
    if [ $? -eq 0 ]; then
        echo "✓ SUCCESS: $data_path (seq_len: $seq_len) - Log: $log_file"
        success_count=$((success_count + 1))
    else
        echo "✗ ERROR: $data_path (seq_len: $seq_len) - Check log: $log_file"
        error_count=$((error_count + 1))
        if [ -z "$failed_datasets" ]; then
            failed_datasets="$data_path($seq_len)"
        else
            failed_datasets="$failed_datasets $data_path($seq_len)"
        fi
    fi
done
