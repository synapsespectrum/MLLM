...
def parse_args():
    ...
    )  # LLAMA2, LLAMA3, GPT2, BERT, GPT2M, GPT2L, GPT2XL, Doc2Vec, ClosedLLM
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_rate2", type=float, default=1e-2, help="mlp learning rate"
    )
    parser.add_argument(
        "--learning_rate3", type=float, default=1e-3, help="proj learning rate"
    )
    parser.add_argument(
        "--prompt_weight", type=float, default=0.01, help="prompt weight"
    )  # please tune this hyperparameter for combining
    parser.add_argument(
        "--pool_type", type=str, default="avg", help="pooling type"
    )  # avg min max attention
    parser.add_argument(
        "--date_name", type=str, default="end_date", help="matching date name in csv"
    )  # mlp linear
    parser.add_argument(
        "--addHisRate", type=float, default=0.5, help="add historical rate"
    )
    parser.add_argument(
        "--init_method",
        type=str,
        default="normal",
        help="init method of combined weight",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_name", type=str, default="result_longterm_forecast", help="save name"
    )
    parser.add_argument(
        "--use_fullmodel", type=int, default=0, help="use full model or just encoder"
    )
    parser.add_argument(
        "--use_closedllm", type=int, default=0, help="use closedllm or not"
    )
    parser.add_argument(
        "--huggingface_token",
        type=str,
        help="your token of huggingface; need for llama3",
    )
    ...
    parser.add_argument(
        "--target", type=str, default="OT", help="Target variable for the dataset"
    )
    parser.add_argument(
        "--feature", type=str, default="M", help="Feature type for the dataset"
    )
...
def save_embeddings(args):
    ...
    train_set = get_dataset(
        args.data_path, "train", args.input_len, args.target, args.feature
    )
    test_set = get_dataset(
        args.data_path, "test", args.input_len, args.target, args.feature
    )
    val_set = get_dataset(
        args.data_path, "val", args.input_len, args.target, args.feature
    )
    ...
    if args.embedding_mode == 0:
        ...
        if args.embedding_mode == 1:  # Using time series data to generate embeddings
            ...
            folder_saved = (
                f"{args.emb_saved_path}/ts/{args.llm_model}/{args.data_path}/"
            )
            ...
            folder_saved = (
                f"{args.emb_saved_path}/ts_txt/{args.llm_model}/{args.data_path}/"
            )
        ...
        for flag, data_loader in data_loaders.items():
            ...
            for i, (x, x_mark, seq_text) in tqdm(
                enumerate(data_loader),
                total=len(data_loader),
                desc="Generating embeddings",
            ):
                if (
                    args.embedding_mode == 1
                ):  # Using time series data to generate embeddings
                    embeddings = gen_prompt_emb.generate_embeddings(
                        x.to(device), x_mark.to(device)
                    )
                else:  # Using both text and time series data to generate embeddings
                    embeddings = gen_prompt_emb.generate_embeddings(
                        x.to(device), x_mark.to(device), seq_text
                    )
