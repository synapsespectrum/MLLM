...
class CrossModal(nn.Module):
    ...
    def forward(
        self, ts_features: torch.Tensor, text_features: torch.Tensor
...
class TSLLMFusionModel(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.fusion_dim = getattr(
            configs, "fusion_dim", 256
        )  # Common dimension after translation
        ...
        self.seq_align_method = getattr(
            configs, "seq_align_method", "mean"
        )  # 'mean', 'max', 'adaptive'
    ...
    def forward(
        ...
        if hasattr(self.ts_model, "forecast"):
            ...
            if ts_output.dim() == 3 and ts_output.size(1) == n_vars:
                ...
                ts_output = ts_output.unsqueeze(2).expand(
                    -1, -1, seq_len, -1
                )  # [batch_size, n_vars, seq_len, d_model]
                ts_output = ts_output.mean(
                    dim=1
                )  # Average across variables: [batch_size, seq_len, d_model]
        else:
            # For placeholder model
            ts_output = self.ts_model(
                x_enc.mean(dim=-1, keepdim=True)
            )  # [batch_size, seq_len, d_model]

        # 2. Translate both modalities to common dimension
        ts_translated = self.ts_translator(
            ts_output
        )  # [batch_size, seq_len, fusion_dim]
        text_translated = self.text_translator(
            prompt_emb
        )  # [batch_size, text_len, fusion_dim]

        # 3. Align sequence lengths
        # ts_aligned, text_aligned = self._align_sequence_length(ts_translated, text_translated)

        # 4. Cross-attention fusion
        fused_output, attention_weights = self.cross_attention(
            ts_translated, text_translated
        )
        ...
        if (
            hasattr(self.configs, "pooling_method")
            and self.configs.pooling_method == "last"
        ):
            fused_features = fused_output[:, -1, :]  # Take last timestep
        else:
            fused_features = fused_output.mean(dim=1)  # Average pooling

        post_fused_output = self.forecasting_head(
            fused_features
        )  # [batch_size, pred_len]
        ...
        if self.use_residual and hasattr(self, "ts_direct_proj"):
            ...
            forecast_output = (
                fusion_weight * post_fused_output + (1 - fusion_weight) * ts_direct
            )
        else:
            ...
            if text_seq_len > ts_seq_len:
                ...
                if remainder > 0:
                    text_features = torch.cat(
                        [text_features, text_features[:, :remainder, :]], dim=1
                    )
