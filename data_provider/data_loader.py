...
class Dataset_Custom(Dataset):
    ...
    def __init__(
        ...
        self.freq = (
            freq  # 'h' for hourly data, 'd' for daily data, 'w' for weekly data, etc.
        )
    ...
    def __read_data__(self):
        ...
        if has_prior and has_dates:
            ...
            cols.remove("end_date")
            df_raw = df_raw[
                ["date"] + cols + [self.target] + ["start_date"] + ["end_date"]
            ]
        else:
        ...
        if self.scale:
            ...
            if has_prior:
                data_prior = self.scaler.transform(
                    df_data_prior.values[:, -1].reshape(-1, 1)
                )
        else:
            data = df_data.values
            if has_prior:
    ...
    def load_embeddings(self):
        ...
        try:
            ...
            print(
                f"Embedding directory {embedding_dir} does not exist. Please check the path or generate embeddings first."
            )
            raise FileNotFoundError(
                f"Embedding directory {embedding_dir} does not exist."
            )
    ...
    def get_prior_y(self, indices):
        ...
        prior_y = np.array(
            [self.data_prior[r_beg:r_end] for r_beg, r_end in zip(r_begins, r_ends)]
        )
        return prior_y
    ...
    def get_prior_y_for_imputation(self, indices):
        ...
        s_ends = s_begins + self.seq_len
        prior_y = np.array(
            [self.data_prior[s_beg:s_end] for s_beg, s_end in zip(s_begins, s_ends)]
        )
        return prior_y
    ...
    def get_text_embeddings(self, indices):
        ...
        text_embeddings = torch.cat(
            [self.text_embeddings[s_end - self.seq_len : s_end] for s_end in s_ends],
            dim=0,
        ).view(bsz, self.seq_len, -1)
    ...
    def get_date(self, indices):
        ...
        x_start_dates = np.array(
            [self.start_date[s_beg:s_end] for s_beg, s_end in zip(s_begins, s_ends)]
        )
        x_end_dates = np.array(
            [self.end_date[s_beg:s_end] for s_beg, s_end in zip(s_begins, s_ends)]
        )
    ...
    def __getitem__(self, index):
        ...
        seq_x = (
            self.data_x[s_begin:s_end, feat_id : feat_id + 1]
            if len(self.data_x.shape) > 1
            else self.data_x[s_begin:s_end].reshape(-1, 1)
        )
        seq_y = (
            self.data_y[r_begin:r_end, feat_id : feat_id + 1]
            if len(self.data_y.shape) > 1
            else self.data_y[r_begin:r_end].reshape(-1, 1)
        )
