...
def parse_args():
    ...
    # Dataset parameters
    parser.add_argument("--divide", type=str, default="train", help="Dataset division: train, test, or val")
    parser.add_argument("--data_path", type=str, default="Environment", help="Dataset name, e.g., 'Farm', 'Health', 'Environment', etc.")
...
def get_dataset(data_path, flag, input_len, output_len, target, feature):
    ...
    return dataset_class(flag=flag,
                         ...
                         features=feature,  # Feature type: M, S, MS
                         target=target,
                         data_path=data_path)
...
def save_embeddings(args):
    ...
    for i, (x, y, x_mark, y_mark) in tqdm(enumerate(data_loader), total=len(data_loader), desc="Generating embeddings"):
