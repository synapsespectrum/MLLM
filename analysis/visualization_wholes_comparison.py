...
def prepare_data(unimodal_df, multimodal_df, tsflib_df):
    ...
    multimodal_df = multimodal_df.rename(
        columns={"input_length": "input_len", "prediction_length": "output_len"}
    )
...
def plot_heatmaps_by_dataset(df, save_dir=None):
    ...
    for dataset in datasets:
        ...
        if not unimodal_data.empty:
            ...
            best_unimodal = (
                unimodal_data.groupby(["input_len", "output_len"])["rmse"]
                .min()
                .reset_index()
            )
            # Create a pivot table for the heatmap
            pivot_uni = best_unimodal.pivot(
                index="input_len", columns="output_len", values="rmse"
            )
            # Plot the heatmap
            ...
            axes[0].text(
                0.5, 0.5, "No unimodal data available", ha="center", va="center"
            )
        ...
        if not multimodal_data.empty:
            # Create a pivot table for the heatmap
            pivot_multi = multimodal_data.pivot(
                index="input_len", columns="output_len", values="rmse"
            )
            # Plot the heatmap
            ...
            axes[1].text(
                0.5, 0.5, "No multimodal data available", ha="center", va="center"
            )
...
def compare_best_models(df, save_dir=None):
    ...
    def get_best_models(df):
        ...
        best_unimodal = (
            df[df["model_type"] == "Unimodal"]
            .groupby("dataset")["rmse"]
            .min()
            .reset_index()
        )
        ...
        best_multimodal = (
            df[df["model_type"] == "Multimodal"]
            .groupby("dataset")["rmse"]
            .min()
            .reset_index()
        )
        best_multimodal = best_multimodal.rename(
            columns={"rmse": "best_multimodal_rmse"}
        )

        # For tsflib models, find the best model for each dataset
        best_tsflib = (
            df[df["model_type"] == "TSFlib"]
            .groupby("dataset")["rmse"]
            .min()
            .reset_index()
        )
        ...
        # Merge the results
        best_models = pd.merge(
            best_unimodal, best_multimodal, on="dataset", how="outer"
        )
        ...
        best_models["multimodal_improvement"] = (
            (best_models["best_unimodal_rmse"] - best_models["best_multimodal_rmse"])
            / best_models["best_unimodal_rmse"]
            * 100
        )

        # Calculate the improvement percentage of tsflib over unimodal
        best_models["tsflib_improvement"] = (
            (best_models["best_unimodal_rmse"] - best_models["best_tsflib_rmse"])
            / best_models["best_unimodal_rmse"]
            * 100
        )
    ...
    rects1 = ax.bar(
        x - width, best_models["best_unimodal_rmse"], width, label="Best Unimodal"
    )
    ...
    def compare_models_for_datasets(df, datasets, metric="mse"):
        ...
        for dataset in datasets:
            ...
            for input_len, output_len in (
                dataset_df[["input_len", "output_len"]].drop_duplicates().values
            ):
                plt.figure(figsize=(14, 8))
                config_df = dataset_df[
                    (dataset_df["input_len"] == input_len)
                    & (dataset_df["output_len"] == output_len)
                ]

                # Get unimodal and tsflib data
                unimodal_data = config_df[
                    config_df["model_type"] == "Unimodal"
                ].sort_values(metric)
                tsflib_data = config_df[
                    config_df["model_type"] == "TSFlib"
                ].sort_values(metric)
                ...
                if not unimodal_data.empty and not tsflib_data.empty:
                    ...
                    all_models = list(unimodal_data["model"]) + list(
                        tsflib_data["model"]
                    )
                ...
                plt.xticks(
                    np.arange(len(all_models)), all_models, rotation=45, ha="right"
                )
                ...
                if save_dir:
                    plt.savefig(
                        f"{save_dir}/comparison_{dataset}_il{input_len}_ol{output_len}.png"
                    )
                else:
                    plt.show()
