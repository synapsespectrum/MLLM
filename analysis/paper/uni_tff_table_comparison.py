...
exp_settings_filter = {
    "Agriculture": {"input_length": 24, "prediction_length": [6, 8, 10, 12]},
    "Climate": {"input_length": 8, "prediction_length": [6, 8, 10, 12]},
    "Economy": {"input_length": 24, "prediction_length": [6, 8, 10, 12]},
    "Energy": {"input_length": 24, "prediction_length": [12, 24, 36, 48]},
    "Environment": {"input_length": 84, "prediction_length": [24, 96, 192, 336]},
    "Health": {"input_length": 36, "prediction_length": [12, 24, 36, 48]},
    "Security": {"input_length": 8, "prediction_length": [6, 8, 10, 12]},
    "SocialGood": {"input_length": 24, "prediction_length": [6, 8, 10, 12]},
    "Traffic": {"input_length": 8, "prediction_length": [6, 8, 10, 12]},
}
...
def prepare_data(unimodal_df, tff_df):
    ...
    combined_df = pd.concat(
        [unimodal_df[common_columns], tff_df[common_columns]], ignore_index=True
    )
    ...
    filtered_df = combined_df[
        combined_df.apply(
            lambda row: (
                row["dataset"] in exp_settings_filter
                and row["input_length"]
                == exp_settings_filter[row["dataset"]]["input_length"]
                and row["prediction_length"]
                in exp_settings_filter[row["dataset"]]["prediction_length"]
            ),
            axis=1,
        )
    ]
...
def generate_detailed_comparison_tables(df, save_dir=None):
    ...
    for dataset in datasets:
        ...
        configs = (
            dataset_df[["input_length", "prediction_length"]].drop_duplicates().values
        )
        ...
        for input_len, output_len in configs:
            ...
            for model_name in unimodel_names:
                univariable_data = config_df[
                    (config_df["model_type"] == "Unimodal")
                    & (config_df["model"] == model_name)
                ]
                for metric in metrics:
...
def generate_aggregate_comparison_table(df, save_dir=None):
    ...
    grouped_df = (
        df.groupby(["dataset", "model_type", "model"])[metrics].mean().reset_index()
    )
    ...
    for dataset in grouped_df["dataset"].unique():
        unimodal_data = grouped_df[
            (grouped_df["dataset"] == dataset)
            & (grouped_df["model_type"] == "Unimodal")
        ]
        tff_data = grouped_df[
            (grouped_df["dataset"] == dataset) & (grouped_df["model_type"] == "TFF")
        ].iloc[0]
        ...
        improvement_mse = (
            (best_baseline["mse"] - tff_data["mse"]) / best_baseline["mse"] * 100
        ).round(2)
        improvement_mae = (
            (best_baseline["mae"] - tff_data["mae"]) / best_baseline["mae"] * 100
        ).round(2)
...
def main():
    ...
    generate_detailed_comparison_tables(combined_df, save_dir=output_dir)

    # Generate aggregate comparison tables
    generate_aggregate_comparison_table(combined_df, save_dir=output_dir)
